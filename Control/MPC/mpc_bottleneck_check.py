...
Python MPC bottleneck check
    - np.vstack, hstack is faster than sparse.vstack, sparse.hstack
    - scipy.linalg.block_diag(ndarray) is faster than sparse.block_diag(sparse matrix)
"""
...
def simulate(init_state, path_x, path_y, path_yaw):
    ...
    for i in range(sim_time):
        ...
        print("mpc time :", time.time()-tic_mpc)
        ...
        if dist < 0.5:
            ...
            if finish_cnt > 15:
                break
        print("dist process time :", time.time() - tic_dist)
